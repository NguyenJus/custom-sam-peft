...
not faithful bf16. This was confirmed live on the Colab T4 via the
`bf16-coercion` capture cell in `notebooks/colab_gpu_tests.ipynb`:

```text
WARNING:custom_sam_peft.runtime._runtime:Requested bfloat16 on a device with
compute capability (7, 5) (< 8.0, where bf16 is emulated); coercing to float16.
Device      : Tesla T4
Capability  : 7.5
...
About 8.5× the 5070 Ti QLoRA step (≈ 0.75 s/step), as expected for the T4's fp16
band. The full figure — and its 5070 Ti counterpart — is recorded in the

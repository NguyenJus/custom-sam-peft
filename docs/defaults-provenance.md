...
| `config/schema.py:TrainHyperparams.cache_trunk_features` | `False` | `index-only` | — | — | Opt-in master toggle for the single-run on-disk trunk-feature replay cache (#300). Off by default: the cache activates only once the three correctness guards pass (trunk frozen, RGB input, aug-off). It is a byte-equivalent (up to the fp16 round-trip) wall-clock optimization with no accuracy trade-off, so the default is a conservative safety gate, not a tuned value. cite: spec `docs/superpowers/specs/2026-06-04-trunk-feature-cache-300-design.md` §2 / §3.5. |
| `config/schema.py:TrainHyperparams.cache_allow_slow_disk` | `False` | `index-only` | — | — | Override for the build-time throughput auto-guard (#300). When False (default) the cache refuses to activate if the measured cold-read throughput of the cache volume is below the DERIVED break-even `feature_bytes / trunk_fwd`; set True only after verifying the disk is fast enough. Off-by-default is the safe choice — the guard probes a real cold read and never trusts the kernel `rotational` flag (it lies under WSL2/VHDX). cite: spec §3.5(c). |
| `config/schema.py:TrainHyperparams.cache_free_disk_fraction` | `0.70` | `# cite: trunk-feature-cache spec §3.5(b)` | `docs/superpowers/specs/2026-06-04-trunk-feature-cache-300-design.md` §3.5(b): refuse to activate when the projected cache size exceeds 70% of free disk on the cache volume. | "when the projected size exceeds **70%** of FREE disk on the cache volume … Cited default `0.70`: leaves ≳30% headroom … 193/918 ≈ 21% ≪ 70%". | Build-time free-disk fit-check ceiling; `0.70` leaves ≥30% headroom for checkpoints, logs, DataLoader spill, and OS while admitting the 193 GiB DFC working set on a 918 GB SSD. Bounded by `Field(gt=0.0, le=1.0)`. |
...
| `presets.py:ADAPTER_DIM_SUM_BY_SCOPE` | `vision=196_608, vision_decoder=233_472, vision_decoder_concept=245_760, decoder_concept=49_152, all=964_129` | `# cite: scripts/_derive_preset_constants.py --scope-survey` | `scripts/_derive_preset_constants.py --scope-survey` (repo root) run against the real SAM 3.1 checkpoint on an RTX 5070 Ti, 2026-06-04 (#301). For each `LoraScope` value it applies LoRA and sums `lora_A.in_features + lora_B.out_features` over every injected peft adapter. | Per-entry inline citation in presets.py: "# derived 2026-06-04 (scope-survey)". Internal consistency: vision (196_608) + decoder_concept (49_152) = vision_decoder_concept (245_760). | Replaces the former `LORA_LAYERS`/`D_IN`/`D_OUT` single-count×average sizing — per-scope exact (captures peft MHA `in_proj`+`out_proj` asymmetry). Consumed by `_adapter_bytes(r, scope)`. |
...
| `presets.py:PresetDecision.alpha` | `32` | `cross-link` | See `config/schema.py:PEFTConfig.alpha` row. | — | The autosize decision's chosen LoRA alpha; pinned to the config/template alpha (32 at the shipped default, 2×r) and co-scaled DOWN only on the warned r-reduction path: `alpha = max(1, round(cfg_alpha * r / cfg_r))`. The field default is a defensive fallback; every constructor sets alpha explicitly. Hardware never raises alpha above the configured value. |
| `presets.py:CACHE_SCHEMA_VERSION` | `4` | `index-only` | — | — | Internal cache versioning integer; not trust-bearing. Incremented when the cache JSON schema changes in a backward-incompatible way. |

...
| `config/schema.py:PEFTConfig.scope` | `"vision_decoder_concept"` | `# tbd: #191` | See `docs/research/2026-06-01-issue-230-peft-adaptation-surface-lit-review.md` §4, §7. | — | Project-chosen SAM 3.1 concept scope (#230). Superset of `vision_decoder` that additionally adapts the `ca_text`/`self_attn` `in_proj` projections so the shipped default can learn niche TEXT concepts; plain `vision_decoder` freezes those. Default flipped from `vision_decoder` (which only hits vision-trunk + decoder-FFN linears) per the #230 research note. No ablation recorded. Tracking via #191. |
...
| `config/schema.py:TrainHyperparams.lr_schedule` | `"plateau"` | `# tbd: #197` | `# tbd: #197` cosine→plateau flip: horizon-mismatch argument (research §3–§4) — real runs are ~160 ep convergence runs where ReduceLROnPlateau + early stop outperforms a fixed cosine that decays to zero regardless of val-mAP. Cosine shape reference (still valid for the `cosine` option): Loshchilov & Hutter 2017, "SGDR: Stochastic Gradient Descent with Warm Restarts", arXiv:1608.03983, ICLR 2017, §3 Eq.(5). | Eq.(5): η_t = η_min + ½(η_max − η_min)(1 + cos(T_cur/T_i · π)) — cosine decay within each restart period. | Default flipped `cosine` → `plateau` in #197. `plateau` pairs ReduceLROnPlateau (rung 1) with early stop (rung 2); `cosine` / `linear` / `constant` remain available. SGDR cosine shape is retained as the reference for the `cosine` option. See [research §2–§4](research/2026-05-30-issue-197-plateau-lr-decay-early-stopping-lit-review.md). |
| `config/schema.py:TrainHyperparams.lr_decay_on_plateau.patience` | `5` | `# cite: framework default` | Keras `ReduceLROnPlateau` default `patience=10`; common practitioner choice of 5 for tighter plateau detection — see [research §2,§7](research/2026-05-30-issue-197-plateau-lr-decay-early-stopping-lit-review.md). | Keras example uses patience=5 as a typical tighter setting (research §2,§7). | plateau mode only (rung 1). |
| `config/schema.py:TrainHyperparams.lr_decay_on_plateau.factor` | `0.1` | `# cite: framework default` | PyTorch `torch.optim.lr_scheduler.ReduceLROnPlateau` default `factor=0.1`. URL: <https://pytorch.org/docs/stable/generated/torch.optim.lr_scheduler.ReduceLROnPlateau.html> — see [research §2,§7](research/2026-05-30-issue-197-plateau-lr-decay-early-stopping-lit-review.md). | "factor (float) – Factor by which the learning rate will be reduced. new_lr = lr * factor. Default: 0.1." | Standard 10× LR reduction per plateau cut. |
| `config/schema.py:TrainHyperparams.lr_decay_on_plateau.min_lr` | `1e-6` | `# tbd: #197` | `# tbd:` learning_rate/100 heuristic (research §7); no internal calibration run recorded. See [research §7](research/2026-05-30-issue-197-plateau-lr-decay-early-stopping-lit-review.md). | — | LR floor; cuts never go below this value. |

...
def read_dcm_with_meta(
    ...
    *,
    ...
    Decode order (spec §8.1):
      1. Modality LUT (apply_modality_lut) — always; converts stored to real units (e.g. HU).
      2. MONOCHROME1 inversion — only when PhotometricInterpretation == "MONOCHROME1".
      3. VOI windowing — only if voi_window override given OR file carries WindowCenter/WindowWidth.
         Override wins over file window.
    ...
    # 2. MONOCHROME1 inversion (min=white convention).
    if getattr(ds, "PhotometricInterpretation", "") == "MONOCHROME1":
        arr = arr.max() - arr

    # 3. VOI windowing — only when there's something to apply.

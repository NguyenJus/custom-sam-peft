"""Tests for custom_sam_peft.models.sam3._resolve_checkpoint_path.
...
(``custom_sam_peft.models.sam3.download_model``) because ``_resolve_checkpoint_path``
will ``from custom_sam_peft.utils.huggingface import download_model`` at module top
and bind the name into ``custom_sam_peft.models.sam3``.
"""
...
from custom_sam_peft.models.sam3 import _resolve_checkpoint_path
...
def test_resolve_checkpoint_returns_path_when_file_present(
    ...
    monkeypatch.setattr("custom_sam_peft.models.sam3.download_model", _fake_dl)
...
def test_resolve_checkpoint_local_dir_none_hints_at_custom_sam_peft_init() -> None:
    cfg = ModelConfig(local_dir=None)
    with pytest.raises(FileNotFoundError) as excinfo:
        _resolve_checkpoint_path(cfg)
    msg = str(excinfo.value)
    assert "local_dir is None" in msg
    assert "custom_sam_peft init" in msg
